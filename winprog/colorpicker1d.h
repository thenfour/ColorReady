...
public:
  ...
  bool PositionFromColor(CPoint& p, const ColorSpec& c) const
  {
    ...
    {
      PositionFromColor(p, c.GetColorant(0));
      r = true;
    }
    return r;
  }
  ...
  bool ColorFromPosition(ColorSpec& ret, long p) const
  {
    ...
    {
      clamp(p, 0, s.cy);
      ret.GetColorant(0) = static_cast<float>(p) / s.cy;
    }
    else
    {
      clamp(p, 0, s.cx);
      ret.GetColorant(0) = static_cast<float>(p) / s.cx;
    }
...
protected:
  void _GenerateSpectrum()
  {
    ...
    if(m_Orientation == Vertical)
    {
      ...
      {
        c.GetColorant(0) = static_cast<float>(y) / SpectrumSize.cy;
    ...
    else
    {
      ...
      {
        c.GetColorant(0) = static_cast<float>(x) / SpectrumSize.cx;
